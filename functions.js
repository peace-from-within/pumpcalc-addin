...
// ---- Pipe_ID ---------------------------------------------------------------
// Exact-match lookup against Pipe_ID sheet (NormID rows x Schedule columns),
// mirroring PipeID_Fn's MATCH(...,0) exact-match semantics. If either match
// fails, VBA/Excel both effectively degrade to the first row/col (IFERROR+1
// in the LAMBDA version) - reproduced here for parity.
function Pipe_ID(NormID, Sch) {
  const schKey = String(Sch);
...
/**
 * Pipe inner diameter lookup (NPS x Schedule).
 * @customfunction PIPE_ID
 * @param {number} normID Nominal pipe size (inch)
 * @param {string} sch Schedule (e.g. "STD", "40", "80")
